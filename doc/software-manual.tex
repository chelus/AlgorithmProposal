...
\newenvironment{OptionTableNoShorthand}[1]{%
	\footnotesize
	\def\arraystretch{1.8}
	\clearOptions
	\begin{longtable}{l<{\makecell[tl]{\optOption}}%
...
for the HEVC project. It applies to version 16.0
of the software.
\end{abstract}
...
%% VUI parameters
%%
\begin{OptionTableNoShorthand}{VUI parameters}
\Option{VuiParametersPresent} \Option{-vui} &
\Default{false} &
...
%% SEI messages
%%
\begin{OptionTableNoShorthand}{SEI messages}
...
\Default{0} &
Enables or disables the insertion of the Knee function SEI messages.
\\
...
\Default{0} &
Indicates that Knee function SEI message cancels the persistance or follows.
\\
\Option{SEIKneeFunctionPersistenceFlag} &
\Default{1} &
Specifies the persistence of the Knee function SEI message.
\\
\Option{SEIKneeFunctionMappingFlag} &
\Default{false} &
Specifies the mapping mode of the Knee function SEI message.
\\
...
\clearpage
\subsection{Range Extensions encoder parameters}

The Range Extension software provides the following additional options.
%%
%% All range extensions options
%%
\begin{OptionTableNoShorthand}{Range Extensions file, I/O and source parameters}
...
\Default{420} &
Specifies the chroma format used in the input file. Permitted values are 400, 420, 422 or 444.
\\

\Option{ChromaFormatIDC}
\Option{-cf} &
\Default{0} &
Specifies the chroma format to use for processing. Permitted values are 400, 420, 422 or 444; the value of 0 indicates that the value of InputChromaFormat should be used instead.
\\
...
\begin{OptionTableNoShorthand}{Range Extensions profiles and constraints}

\Option{Profile} &
\Default{\None} &
The Profile option of HM has been extended to include "main-RExt"
\\

\Option{MaxBitDepthConstraint} &
\Default{0} &
Specifies the value to use to derive the general_max_bit_depth constraint flags for RExt profiles; when 0, use $\max(InternalBitDepth, InternalBitDepthC)$
\\

\Option{MaxChromaFormatConstraint} &
\Default{0} &
Specifies the chroma-format to use for the general profile constraints for RExt profiles; when 0, use the value of ChromaFormatIDC.
\\

\Option{IntraConstraintFlag} &
\Default{0} &
Specifies the value of general_intra_constraint_flag to use for RExt profiles.
\\
...
\begin{OptionTableNoShorthand}{Range Extensions tool parameters}
...
\Default{false} &
Specifies the value of high_precision_prediction_weighting_flag.
\\

\Option{SingleComponentLoopInterSearch} &
\Default{false} &
When true, specifies the use of the single-component loop inter search, rather than the multi-pass approach. It is hoped that this option can be removed in a future revision.
\\

\Option{IntraBlockCopyEnabled} &
\Default{false} &
When true, specifies the use of the intra block copy tool.
\\

\Option{IntraBlockCopyFastSearch} &
\Default{true} &
When true, specifies the use of restricted searching for intra block copy motion vectors to reduce the encoding time.
\\

\Option{CrossComponentPrediction} &
\Default{true} &
When true, specifies the use of the cross component prediction tool.
\\
...
\Default{0}
\Default{0} &
Specifies the shift to apply to the SAO parameters.
\\

\Option{TransformSkipLog2MaxSize} &
\Default{2} &
Specifies the maximum TU size for which transform-skip can be used; the minimum value is 2.
\\

\Option{ImplicitResidualDPCM} &
\Default{false} &
When true, specifies the use of the implicitly signalled residual RDPCM tool (for intra)
\\

\Option{ExplicitResidualDPCM} &
\Default{false} &
When true, specifies the use of the explicitly signalled residual RDPCM tool (for intra-block-copy and inter)
\\

\Option{ResidualRotation} &
\Default{false} &
When true, specifies the use of the residual rotation tool
\\

\Option{SingleSignificanceMapContext} &
\Default{false} &
When true, specifies the use of a single significance map context for transform-skipped and transquant-bypassed TUs.
\\

\Option{GolombRiceParameterAdaptation} &
\Default{false} &
When true, enable the adaptation of the Golomb-Rice parameter over the course of each slice.
\\
...
\Default{true} &
When true, enable intra reference smoothing.
\\
...
\subsection{Using the HM decoder}
\begin{verbatim}
...
\end{OptionTable}
\clearpage
\subsection{Using the RExt decoder}

\begin{OptionTableNoShorthand}{Additional RExt decoder options}
